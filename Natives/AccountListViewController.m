...
- (void)viewDidLoad {
  ...
  // Add skin header if an account is already selected
...
- (UIView *)buildSkinHeaderView {
  ...
  // Skin body image — bottom-anchored so it “stands” on the panel floor
  UIImageView *skinView = [[UIImageView alloc] init];
  skinView.contentMode = UIViewContentModeScaleAspectFit;
  // Nearest-neighbor for pixel-perfect Minecraft skin rendering
  ...
  // Load skin body from mc-heads.net
  ...
  // Subtitle — Xbox gamertag or account type
  ...
  // Account type badge (● Microsoft / ● Local account)
  ...
  // Layout — stack vertically centered in right panel
  [NSLayoutConstraint activateConstraints:@[
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  ```
    // Refresh header to reflect new active account
    if (BaseAuthenticator.current != nil) {
        self.tableView.tableHeaderView = [self buildSkinHeaderView];
    }
  ```
...
- (void)callbackMicrosoftAuth:(id)status success:(BOOL)success forCell:(UITableViewCell *)cell {
  ...
  if (self.whenItemSelected) self.whenItemSelected();
  [self removeActivityIndicatorFrom:cell];
  // Refresh skin header to show newly selected account
  self.tableView.tableHeaderView = [self buildSkinHeaderView];
  [self dismissOrPop];
  }
  }

/// Handles both pushed (navigation) and presented (popover) cases
